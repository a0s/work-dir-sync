...
# "<name>=<backend type>" pairs, filled in by storage() — several decisions
# below (which flags help, how much parallelism is safe) depend on the backend.
STORAGE_TYPES=""
...
# What bisync compares to decide two files are identical. Size and modtime are
# cheap but not exact: two versions of the same file that happen to share a size
# and land in the same modify-window look equal, and two machines writing at
# once can then drift apart without anything noticing.
RCLONE_COMPARE="size,modtime"

# On SSH servers content comparison is worth its price: the server hashes the
# file locally (md5sum/sha1sum) instead of sending it, so a difference is always
# seen. Object stores are left on size,modtime — there a hash means an extra
# billed request per object.
# Note: rclone runs one command per hash, and with ssh= (an external ssh binary)
# that is one new SSH connection per file unless ControlMaster is on — see the
# README. Set this to "$RCLONE_COMPARE" to opt out.
...
# ...and the same for SSH servers, which need their own, much smaller, numbers:
# rclone opens one SSH connection per transfer and per checker, while sshd
# allows ten sessions per connection (MaxSessions) and twice that in total
# (MaxStartups) before it starts refusing them. 4+4 stays comfortably below
# both; raise them only if you control the server's sshd_config.
SFTP_TRANSFERS=4
SFTP_CHECKERS=4
...
storage() {
  ...
  # --obscure: password fields (sftp pass=, key_file_pass=) have to be stored
  # scrambled or rclone will not read them back. rclone guesses on its own, but
  # guesses wrong for anything that already looks base64 — which a password
  # generated by a password manager usually does. Non-password fields (an S3
  # secret_access_key among them) are left alone either way.
  if rclone config create "$name" "$type" ${args[@]+"${args[@]}"} --obscure --non-interactive >/dev/null 2>&1; then
    KNOWN_STORAGES="$KNOWN_STORAGES $name"
    STORAGE_TYPES="$STORAGE_TYPES $name=$type"
    return 0
  fi
...
# The backend behind a remote: "r2:bucket/sub" -> "s3", "srv:/data" -> "sftp".
# Normally answered from what define_storages() just declared; the rclone config
# is only consulted for remotes defined outside of it.
remote_type() {
  local name="${1%%:*}" kv
...
# Can the far side produce checksums? On SFTP that depends on the server having
# md5sum/sha1sum in $PATH (and on disable_hashcheck not being set), so it has to
# be asked rather than assumed — with `--compare checksum` against a remote that
# has no hashes, bisync aborts on every run.
#
# The answer costs a connection, so it is asked once per pair and remembered;
# `status.sh --resync-force` forgets it again.
remote_has_hashes() {
  local remote="$1" key="$2" file="$STATE_DIR/hashes-$key" answer
  if [ -s "$file" ]; then
    answer="$(cat "$file")"
  else
    if rclone backend features "${remote%%:*}:" $EXTRA_RCLONE_FLAGS 2>/dev/null |
       grep -qE '"(md5|sha1)"'; then
      answer=yes
      log "[$remote] server provides checksums — comparing by content"
    else
      answer=no
      log "[$remote] server provides no checksums — comparing by size and modtime only"
    fi
...
run_bisync() {
  ...
  # What the other side is decides how hard we may push it. An object store
  # bills per request and answers a recursive listing in one; an SSH server
  # bills nothing but hands out a limited number of connections.
  local rtype transfers="$RCLONE_TRANSFERS" checkers="$RCLONE_CHECKERS"
  local compare="$RCLONE_COMPARE"
  local tuning=()
  rtype="$(remote_type "$remote")"
  case "$rtype" in
    sftp|local)
      if [ "$rtype" = "sftp" ]; then
        transfers="$SFTP_TRANSFERS"; checkers="$SFTP_CHECKERS"
        # A server with no md5sum/sha1sum cannot be compared by content, and
        # bisync would abort every cycle instead of quietly using less.
        remote_has_hashes "$remote" "$key" && compare="$SFTP_COMPARE"
      fi
      ;;
    *)
...
# --------------------------------------------------------------- change marker
#
# Polling the remote with a full listing costs ~one class A request per 1000
...
#
# On an SSH server nothing is billed per request, but the same marker still
# earns its keep: reading one small file over an existing connection is orders
# of magnitude cheaper than walking a deep tree with one round trip per
# directory, which is what SFTP listings cost.
...
remote_marker_changed() {
  local remote="$1" key="$2" rtype="${3:-}"
  local seen_file="$STATE_DIR/marker-$key"
  local cur flags=()
  # --s3-no-head-object: skip the HEAD rclone would otherwise do before the GET,
  # turning the check into exactly one class B request. S3 only — other backends
  # do not charge per request, and rclone rejects nothing but still ignores it.
  [ "$rtype" = "s3" ] && flags=(--s3-no-head-object)
  cur="$(rclone cat "$remote/$MARKER_PATH" ${flags[@]+"${flags[@]}"} $EXTRA_RCLONE_FLAGS 2>/dev/null)"
...
pair_worker() {
  ...
  local rtype; rtype="$(remote_type "$remote")"
  ...
  log "[$remote] worker started (${rtype:-unknown backend}), watching $dir"
  ...
  while :; do
    ...
    if [ "$pending" = "1" ]; then
      ...
      last_marker="$now"
      remote_marker_changed "$remote" "$key" "$rtype" && reason="remote update"
    fi
    ...
    if [ -n "$reason" ]; then
      ...
      if run_bisync "$remote" "$dir"; then
        if run_touched_anything "$key"; then
          ...
          # push_publish succeeds trivially when push is off, which used to make
          # every line claim a notification nobody sent
          if push_enabled && push_publish "$remote" "$(cat "$STATE_DIR/marker-$key" 2>/dev/null)"; then
